...
class trainBuild:
	...
	def getValues(self):
		nrc = pd.read_csv("/home/j/NLP traitsPredictor/process/better.csv", header = None, index_col = False)
		for index, row in nrc.iterrows():
			self.better[row[0]] = row[1:].values.tolist()
			self.word.append(row[0])
		self.data = pd.read_csv("//home/j/NLP traitsPredictor/mypersonality_final.csv")
	...
	def getStatusProcessed(self):
		...
		self.data.to_csv("/home/j/NLP traitsPredictor/training_data/trainV1.csv", index = False, header = False)
		mat = [] ## store processed numerical vectors
		for index, row in self.data.iterrows():
			mat.append(status[index] + row[2:].values.tolist())
		## write to file
		pd.DataFrame(mat).to_csv("//home/j/NLP traitsPredictor/training_data/trainV2.csv", index = False, header = False)
x = trainBuild()
x.getValues()
